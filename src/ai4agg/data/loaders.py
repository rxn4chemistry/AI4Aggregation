...
def make_reaction_set(data_path: Path) -> pd.DataFrame:
    data = pd.read_csv(data_path, index_col=0)
    data = data.reset_index(drop=True)
    return data

# Whole Peptide set: Only consider whole peptides
def make_whole_peptide_set(data_path: Path) -> pd.DataFrame:
...
def make_shuffled_peptide_set(data_path: Path) -> pd.DataFrame:
...
def make_wof_peptide_set(
    data_path: Path, start: int, end: int, drop: bool = False
) -> pd.DataFrame:
    data = make_reaction_set(data_path)
    chunks = list()
    for serial in data["serial"].unique():
        subset = data[data["serial"] == serial]
        chunks.append(get_aggregation_label_wof(subset.copy(), start, end, drop))

    return pd.concat(chunks)


# Aggregation Point: Drop amino acids after aggregation point
def make_agg_point_peptide_set(data_path: Path) -> pd.DataFrame:
    data = make_reaction_set(data_path)
